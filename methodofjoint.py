...
for point, (fx, fy) in eforce.items():
    ...
    if fy != 0:
        if fy < 0:
            ...
            fya = 1  
    ax.quiver(x, y, fxa, fya, angles='xy', scale_units='xy', scale=1, color='#4A26AB', width=0.005)
    ax.text(x + 0.1 * fx, y + 0.1 * fy, f'({fx:.2f}, {fy:.2f})', fontsize=10, ha='left')
...
for point, (rx, ry) in reaction.items():
    ...
    if rx == 1:
        ...
        ax.text(x + 1 * rxa, y, f'{m_s[index][0]:.2f}', fontsize=10, ha='center', va='center')
        index += 1
    ...
    if ry == 1:
        ...
        ax.text(x, y + 1 * rya, f'{m_s[index][0]:.2f}', fontsize=10, ha='center', va='center')
        index += 1
